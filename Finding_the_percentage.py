...
#You are required to save the record in a dictionary data type. The user then enters a student's name. Output the average percentage 
...
while i<n:
    a = raw_input().split(' ')
    perc = (float(a[1]) + float(a[2]) + float(a[3]))/3.0
    b[a[0]] = perc
    i += 1
name = raw_input()
if(name in b):
